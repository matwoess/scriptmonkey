...
import type { Script, UpdateInfo, ExtensionMessage } from '../types';

async function send<T = unknown>(message: ExtensionMessage): Promise<T> {
  const response = (await chrome.runtime.sendMessage(message)) as { error?: string } | T;
...
export default function App() {
  ...
  const [userScriptsAvailable, setUserScriptsAvailable] = useState<boolean>(true);
  const [currentUrl, setCurrentUrl] = useState<string>('');
  const [updatesById, setUpdatesById] = useState<Map<string, UpdateInfo>>(new Map());
  ...
  const loadData = async () => {
    ...
    const status = await send<{ userScriptsAvailable: boolean }>({ type: 'getStatus' });
  ...
  const refreshUpdates = async () => {
    ...
    const updates: UpdateInfo[] = await send<UpdateInfo[]>({ type: 'checkForUpdates' });
  ...
  const availableUpdates = Array.from(updatesById.values()).filter(u => u.hasUpdate).length;
  const checkableUpdates = Array.from(updatesById.values()).filter(u => u.canUpdate).length;
  ...
  return (
    <>
      ...
      {addError && (
        <div className="banner" style={{ color: '#ef4444', borderColor: '#ef4444', backgroundColor: 'rgba(239, 68, 68, 0.1)' }}>
      ...
      {canUpdateAny && (
        <div className="toolbar">
          <div className="toolbar-info" id="update-status">
            {isCheckingUpdates ? 'Checking...' : updateError ? updateError : hasCheckedUpdates ? (checkableUpdates ? `${availableUpdates} update${availableUpdates === 1 ? '' : 's'} available` : 'No update URLs') : ''}
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button className="btn" onClick={() => void handleCheckUpdates()} disabled={isCheckingUpdates}>
              Check for updates
            </button>
            {hasCheckedUpdates && availableUpdates > 0 && (
              <button className="btn btn-primary" onClick={() => void handleUpdateAll()} disabled={isUpdatingAll}>
                Update all
              </button>
            )}
          </div>
      ...
      <div id="active-section">
        ...
        <div id="active-list">
          {activeScripts.length > 0 ? (
            activeScripts.map(script => <ScriptItem key={script.id} script={script} updateInfo={updatesById.get(script.id)} hasCheckedUpdates={hasCheckedUpdates} onToggle={() => { void handleToggle(script.id); }} onRemove={() => handleRemove(script)} onUpdate={() => { void handleUpdate(script.id); }} />)
          ) : (
      ...
      {otherScripts.length > 0 && (
        <div id="other-section">
          ...
          <div id="other-list">
            {otherScripts.map(script => <ScriptItem key={script.id} script={script} updateInfo={updatesById.get(script.id)} hasCheckedUpdates={hasCheckedUpdates} onToggle={() => { void handleToggle(script.id); }} onRemove={() => handleRemove(script)} onUpdate={() => { void handleUpdate(script.id); }} />)}
          </div>
        </div>
      )}

      <div className="actions">
        <button className="btn btn-primary" id="btn-add-file" onClick={() => { fileInputRef.current?.click(); }}>
          + Add file
        </button>
        <input type="file" id="file-input" accept=".js,.user.js" multiple hidden ref={fileInputRef} onChange={handleAddFiles} />
      </div>

      <div id="confirm-modal" className={`modal-overlay ${confirmModal?.show ? 'show' : ''}`}>
        <div className="modal">
          <h3 id="confirm-title">{confirmModal?.title}</h3>
          <p id="confirm-message">{confirmModal?.message}</p>
          <div className="modal-actions">
            <button className="btn" id="confirm-cancel" onClick={() => setConfirmModal(null)}>Cancel</button>
            <button className="btn btn-primary" id="confirm-ok" onClick={confirmModal?.onConfirm}>OK</button>
          </div>
        </div>
      </div>
    </>
...
function ScriptItem({ script, updateInfo, hasCheckedUpdates, onToggle, onRemove, onUpdate }: { script: Script; updateInfo?: UpdateInfo; hasCheckedUpdates: boolean; onToggle: () => void; onRemove: () => void; onUpdate: () => void }) {
  ...
  return (
    <div className="script-item">
      ...
      {hasCheckedUpdates && updateInfo?.hasUpdate && (
        <button className="btn btn-update" onClick={onUpdate}>Update</button>
      )}
      <label className="toggle">
        <input type="checkbox" checked={script.enabled} onChange={onToggle} />
        <span className="slider"></span>
      </label>
      <button className="btn-remove" title="Remove" onClick={onRemove}>&times;</button>
    </div>
  );
}
